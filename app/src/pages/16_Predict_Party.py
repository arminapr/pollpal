...
questions = {
    ...
    9: "22. Next, do you favor, oppose, or neither favor nor oppose the government trying to reduce the difference in incomes between the richest and poorest households?",
...
if len(user_input) == 14:
    ...
    if st.button("Retrain Model"):
        train_query = 'http://web-api:4000/m/ml_model/train'
        response = requests.get(train_query)
        st.write(response)
        results = response.json()
        st.write(f'Retraining Status: {results.get("result", "Unknown")}')
else:
